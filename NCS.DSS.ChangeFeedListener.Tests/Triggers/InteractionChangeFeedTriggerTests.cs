...
{
    public class InteractionChangeFeedTriggerTests
    {
        private Mock<ILogger<InteractionChangeFeedTrigger.InteractionChangeFeedTrigger>> _logger;
        
        private Mock<IChangeFeedListenerServiceBusClient> _serviceBusClient;
        private InteractionChangeFeedTrigger.InteractionChangeFeedTrigger _interactionTrigger;
        private string _jsonDocument = string.Empty;

        [SetUp]
        public void Setup()
        {
            _logger = new Mock<ILogger<InteractionChangeFeedTrigger.InteractionChangeFeedTrigger>>();
            
            _serviceBusClient = new Mock<IChangeFeedListenerServiceBusClient>();
            _interactionTrigger = new InteractionChangeFeedTrigger.InteractionChangeFeedTrigger(_serviceBusClient.Object,  _logger.Object);
        ...
        [Test]
        public async Task Run_CallsServiceBusClientMethodWithModelPropertyIsInteractionAsTrue_WhenDocumentIsValid()
        {
            //Arrange
            ...
            //Act
            await _interactionTrigger.Run(documentsList.AsReadOnly());

            //Assert
            _serviceBusClient.Verify(s => s.SendChangeFeedMessageAsync(It.IsAny<string>(), It.Is<ChangeFeedMessageModel>(m => m.IsInteraction == true)));
        }
