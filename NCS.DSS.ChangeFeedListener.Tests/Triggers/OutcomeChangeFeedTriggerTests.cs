...
{
    public class OutcomeChangeFeedTriggerTests
    {
        private Mock<ILogger<OutcomeChangeFeedTrigger.OutcomeChangeFeedTrigger>> _logger;
        
        private Mock<IChangeFeedListenerServiceBusClient> _serviceBusClient;
        private OutcomeChangeFeedTrigger.OutcomeChangeFeedTrigger _outcomeTrigger;
        private string _jsonDocument = string.Empty;

        [SetUp]
        public void Setup()
        {
            _logger = new Mock<ILogger<OutcomeChangeFeedTrigger.OutcomeChangeFeedTrigger>>();
            
            _serviceBusClient = new Mock<IChangeFeedListenerServiceBusClient>();
            _outcomeTrigger = new OutcomeChangeFeedTrigger.OutcomeChangeFeedTrigger(_serviceBusClient.Object,  _logger.Object);
        ...
        [Test]
        public async Task Run_CallsServiceBusClientMethodWithModelPropertyIsOutcomeAsTrue_WhenDocumentIsValid()
        {
            //Arrange
            ...
            //Act
            await _outcomeTrigger.Run(documentsList.AsReadOnly());

            //Assert
            _serviceBusClient.Verify(s => s.SendChangeFeedMessageAsync(It.IsAny<string>(), It.Is<ChangeFeedMessageModel>(m => m.IsOutcome == true)));
        }
