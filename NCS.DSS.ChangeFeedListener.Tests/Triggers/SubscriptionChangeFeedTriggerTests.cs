...
{
    public class SubscriptionChangeFeedTriggerTests
    {
        private Mock<ILogger<SubscriptionChangeFeedTrigger.SubscriptionChangeFeedTrigger>> _logger;
        
        private Mock<IChangeFeedListenerServiceBusClient> _serviceBusClient;
        private SubscriptionChangeFeedTrigger.SubscriptionChangeFeedTrigger _subscriptionTrigger;
        private string _jsonDocument = string.Empty;

        [SetUp]
        public void Setup()
        {
            _logger = new Mock<ILogger<SubscriptionChangeFeedTrigger.SubscriptionChangeFeedTrigger>>();
            
            _serviceBusClient = new Mock<IChangeFeedListenerServiceBusClient>();
            _subscriptionTrigger = new SubscriptionChangeFeedTrigger.SubscriptionChangeFeedTrigger(_serviceBusClient.Object,  _logger.Object);
        ...
        [Test]
        public async Task Run_CallsServiceBusClientMethodWithModelPropertyIsSubscriptionAsTrue_WhenDocumentIsValid()
        {
            //Arrange
            ...
            //Act
            await _subscriptionTrigger.Run(documentsList.AsReadOnly());

            //Assert
            _serviceBusClient.Verify(s => s.SendChangeFeedMessageAsync(It.IsAny<string>(), It.Is<ChangeFeedMessageModel>(m => m.IsSubscription == true)));
        }
