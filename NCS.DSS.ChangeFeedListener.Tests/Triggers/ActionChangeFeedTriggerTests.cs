...
namespace NCS.DSS.ChangeFeedListener.Tests.Triggers
{
    public class ActionChangeFeedTriggerTests
    {
        ...
        [Test]
        public async Task Run_CallsServiceBusClientMethodWithModelPropertyIsActionAsTrue_WhenDocumentIsValid()
        {
            //Arrange
            ...
            //Act
            await _actionTrigger.Run(_documentsList.AsReadOnly());

            //Assert
            _serviceBusClient.Verify(s => s.SendChangeFeedMessageAsync(It.IsAny<string>(), It.Is<ChangeFeedMessageModel>(m => m.IsAction == true)));
        }
