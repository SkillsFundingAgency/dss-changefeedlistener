...
{
    public class EmploymentProgressionChangeFeedTriggerTests
    {
        private Mock<ILogger<EmploymentProgressionChangeFeedTrigger.EmploymentProgressionChangeFeedTrigger>> _logger;
        
        private Mock<IChangeFeedListenerServiceBusClient> _serviceBusClient;
        private EmploymentProgressionChangeFeedTrigger.EmploymentProgressionChangeFeedTrigger _employmentProgressionTrigger;
        private string _jsonDocument = string.Empty;

        [SetUp]
        public void Setup()
        {
            _logger = new Mock<ILogger<EmploymentProgressionChangeFeedTrigger.EmploymentProgressionChangeFeedTrigger>>();
            
            _serviceBusClient = new Mock<IChangeFeedListenerServiceBusClient>();
            _employmentProgressionTrigger = new EmploymentProgressionChangeFeedTrigger.EmploymentProgressionChangeFeedTrigger(_serviceBusClient.Object,  _logger.Object);
        ...
        [Test]
        public async Task Run_CallsServiceBusClientMethodWithModelPropertyIsEmploymentProgressionAsTrue_WhenDocumentIsValid()
        {
            //Arrange
            ...
            //Act
            await _employmentProgressionTrigger.Run(documentsList.AsReadOnly());

            //Assert
            _serviceBusClient.Verify(s => s.SendChangeFeedMessageAsync(It.IsAny<string>(), It.Is<ChangeFeedMessageModel>(m => m.IsEmploymentProgression == true)));
        }
