...
{
    public class WebChatChangeFeedTriggerTests
    {
        private Mock<ILogger<WebChatChangeFeedTrigger.WebChatChangeFeedTrigger>> _logger;
        
        private Mock<IChangeFeedListenerServiceBusClient> _serviceBusClient;
        private WebChatChangeFeedTrigger.WebChatChangeFeedTrigger _webChatTrigger;
        private string _jsonDocument = string.Empty;

        [SetUp]
        public void Setup()
        {
            _logger = new Mock<ILogger<WebChatChangeFeedTrigger.WebChatChangeFeedTrigger>>();
            
            _serviceBusClient = new Mock<IChangeFeedListenerServiceBusClient>();
            _webChatTrigger = new WebChatChangeFeedTrigger.WebChatChangeFeedTrigger(_serviceBusClient.Object,  _logger.Object);
        ...
        [Test]
        public async Task Run_CallsServiceBusClientMethodWithModelPropertyIsWebChatAsTrue_WhenDocumentIsValid()
        {
            //Arrange
            ...
            //Act
            await _webChatTrigger.Run(documentsList.AsReadOnly());

            //Assert
            _serviceBusClient.Verify(s => s.SendChangeFeedMessageAsync(It.IsAny<string>(), It.Is<ChangeFeedMessageModel>(m => m.IsWebChat == true)));
        }
