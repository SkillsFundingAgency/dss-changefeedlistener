...
{
    public class ContactChangeFeedTriggerTests
    {
        private Mock<ILogger<ContactChangeFeedTrigger.ContactChangeFeedTrigger>> _logger;
        
        private Mock<IChangeFeedListenerServiceBusClient> _serviceBusClient;
        private ContactChangeFeedTrigger.ContactChangeFeedTrigger _contactTrigger;
        private string _jsonDocument = string.Empty;

        [SetUp]
        public void Setup()
        {
            _logger = new Mock<ILogger<ContactChangeFeedTrigger.ContactChangeFeedTrigger>>();
            
            _serviceBusClient = new Mock<IChangeFeedListenerServiceBusClient>();
            _contactTrigger = new ContactChangeFeedTrigger.ContactChangeFeedTrigger(_serviceBusClient.Object,  _logger.Object);
        ...
        [Test]
        public async Task Run_CallsServiceBusClientMethodWithModelPropertyIsContactAsTrue_WhenDocumentIsValid()
        {
            //Arrange
            ...
            //Act
            await _contactTrigger.Run(documentsList.AsReadOnly());

            //Assert
            _serviceBusClient.Verify(s => s.SendChangeFeedMessageAsync(It.IsAny<string>(), It.Is<ChangeFeedMessageModel>(m => m.IsContact == true)));
        }
