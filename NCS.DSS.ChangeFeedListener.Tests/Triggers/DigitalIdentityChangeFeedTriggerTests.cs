...
namespace NCS.DSS.ChangeFeedListener.Tests.Triggers
{
    public class DigitalIdentityChangeFeedTriggerTests
    {
        ...
        public void Setup()
        {
            ...
            _digitalIdentityTrigger = new DigitalIdentityChangeFeedTrigger.DigitalIdentityChangeFeedTrigger(_serviceBusClient.Object, _loggerHelper.Object, _logger.Object);
        ...
        public async Task Run_CallsServiceBusClientMethodWithModelPropertyIsDigitalIdentityAsTrue_WhenDocumentIsValid()
        {
            ...
            //Act
            await _digitalIdentityTrigger.Run(documentsList.AsReadOnly());

            //Assert
            _serviceBusClient.Verify(s => s.SendChangeFeedMessageAsync(It.IsAny<Document>(), It.Is<ChangeFeedMessageModel>(m => m.IsDigitalIdentity == true)));
        }
        ...
        public async Task Run_LogsInformation_WhenDocumentIsValid()
        {
            //Arrange
            var document = JsonConvert.DeserializeObject<Document>(_jsonDocument);
            var logMessage = string.Format("Attempting to send digital identity document id: {0} to service bus queue", document.Id);
