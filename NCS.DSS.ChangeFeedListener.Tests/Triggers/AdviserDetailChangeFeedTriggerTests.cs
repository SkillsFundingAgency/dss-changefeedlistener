...
{
    public class AdviserDetailChangeFeedTriggerTests
    {
        private Mock<ILogger<AdviserDetailChangeFeedTrigger.AdviserDetailChangeFeedTrigger>> _logger;
        
        private Mock<IChangeFeedListenerServiceBusClient> _serviceBusClient;
        private AdviserDetailChangeFeedTrigger.AdviserDetailChangeFeedTrigger _adviserDetailTrigger;
        private string _jsonDocument = string.Empty;

        [SetUp]
        public void Setup()
        {
            _logger = new Mock<ILogger<AdviserDetailChangeFeedTrigger.AdviserDetailChangeFeedTrigger>>();
            
            _serviceBusClient = new Mock<IChangeFeedListenerServiceBusClient>();
            _adviserDetailTrigger = new AdviserDetailChangeFeedTrigger.AdviserDetailChangeFeedTrigger(_serviceBusClient.Object,  _logger.Object);
        ...
        [Test]
        public async Task Run_CallsServiceBusClientMethodWithModelPropertyIsAdviserDetailAsTrue_WhenDocumentIsValid()
        {
            //Arrange
            ...
            //Act
            await _adviserDetailTrigger.Run(documentsList.AsReadOnly());

            //Assert
            _serviceBusClient.Verify(s => s.SendChangeFeedMessageAsync(It.IsAny<string>(), It.Is<ChangeFeedMessageModel>(m => m.IsAdviserDetail == true)));
        }
