...
{
    public class CollectionChangeFeedTriggerTests
    {
        private Mock<ILogger<CollectionChangeFeedTrigger.CollectionChangeFeedTrigger>> _logger;
        
        private Mock<IChangeFeedListenerServiceBusClient> _serviceBusClient;
        private CollectionChangeFeedTrigger.CollectionChangeFeedTrigger _collectionTrigger;
        private string _jsonDocument = string.Empty;

        [SetUp]
        public void Setup()
        {
            _logger = new Mock<ILogger<CollectionChangeFeedTrigger.CollectionChangeFeedTrigger>>();
            
            _serviceBusClient = new Mock<IChangeFeedListenerServiceBusClient>();
            _collectionTrigger = new CollectionChangeFeedTrigger.CollectionChangeFeedTrigger(_serviceBusClient.Object,  _logger.Object);
        ...
        [Test]
        public async Task Run_CallsServiceBusClientMethodWithModelPropertyIsCollectionAsTrue_WhenDocumentIsValid()
        {
            //Arrange
            ...
            //Act
            await _collectionTrigger.Run(documentsList.AsReadOnly());

            //Assert
            _serviceBusClient.Verify(s => s.SendChangeFeedMessageAsync(It.IsAny<string>(), It.Is<ChangeFeedMessageModel>(m => m.IsCollection == true)));
        }
