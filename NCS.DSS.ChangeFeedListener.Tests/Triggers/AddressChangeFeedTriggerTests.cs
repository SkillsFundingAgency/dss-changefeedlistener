...
{
    public class AddressChangeFeedTriggerTests
    {
        private Mock<ILogger<AddressChangeFeedTrigger.AddressChangeFeedTrigger>> _logger;
        
        private Mock<IChangeFeedListenerServiceBusClient> _serviceBusClient;
        private AddressChangeFeedTrigger.AddressChangeFeedTrigger _addressTrigger;
        ...
        public void Setup()
        {
            _logger = new Mock<ILogger<AddressChangeFeedTrigger.AddressChangeFeedTrigger>>();
            
            _serviceBusClient = new Mock<IChangeFeedListenerServiceBusClient>();
            _addressTrigger = new AddressChangeFeedTrigger.AddressChangeFeedTrigger(_serviceBusClient.Object,  _logger.Object);
        ...
        [Test]
        public async Task Run_CallsServiceBusClientMethodWithModelPropertyIsAddressAsTrue_WhenDocumentIsValid()
        {
            //Arrange           
            ...
            //Act
            await _addressTrigger.Run(_documentsList.AsReadOnly());

            //Assert
            _serviceBusClient.Verify(s => s.SendChangeFeedMessageAsync(It.IsAny<string>(), It.Is<ChangeFeedMessageModel>(m => m.IsAddress == true)));
        }
