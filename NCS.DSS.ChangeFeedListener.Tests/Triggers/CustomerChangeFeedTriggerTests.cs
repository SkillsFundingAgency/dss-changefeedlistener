...
{
    public class CustomerChangeFeedTriggerTests
    {
        private Mock<ILogger<CustomerChangeFeedTrigger.CustomerChangeFeedTrigger>> _logger;
        
        private Mock<IChangeFeedListenerServiceBusClient> _serviceBusClient;
        private CustomerChangeFeedTrigger.CustomerChangeFeedTrigger _customerTrigger;
        private string _jsonDocument = string.Empty;

        [SetUp]
        public void Setup()
        {
            _logger = new Mock<ILogger<CustomerChangeFeedTrigger.CustomerChangeFeedTrigger>>();
            
            _serviceBusClient = new Mock<IChangeFeedListenerServiceBusClient>();
            _customerTrigger = new CustomerChangeFeedTrigger.CustomerChangeFeedTrigger(_serviceBusClient.Object,  _logger.Object);
        ...
        [Test]
        public async Task Run_CallsServiceBusClientMethodWithModelPropertyIsCustomerAsTrue_WhenDocumentIsValid()
        {
            //Arrange
            ...
            //Act
            await _customerTrigger.Run(documentsList.AsReadOnly());

            //Assert
            _serviceBusClient.Verify(s => s.SendChangeFeedMessageAsync(It.IsAny<string>(), It.Is<ChangeFeedMessageModel>(m => m.IsCustomer == true)));
        }
