...
{
    public class GoalChangeFeedTriggerTests
    {
        private Mock<ILogger<GoalChangeFeedTrigger.GoalChangeFeedTrigger>> _logger;
        
        private Mock<IChangeFeedListenerServiceBusClient> _serviceBusClient;
        private GoalChangeFeedTrigger.GoalChangeFeedTrigger _goalTrigger;
        private string _jsonDocument = string.Empty;

        [SetUp]
        public void Setup()
        {
            _logger = new Mock<ILogger<GoalChangeFeedTrigger.GoalChangeFeedTrigger>>();
            
            _serviceBusClient = new Mock<IChangeFeedListenerServiceBusClient>();
            _goalTrigger = new GoalChangeFeedTrigger.GoalChangeFeedTrigger(_serviceBusClient.Object,  _logger.Object);
        ...
        [Test]
        public async Task Run_CallsServiceBusClientMethodWithModelPropertyIsGoalAsTrue_WhenDocumentIsValid()
        {
            //Arrange
            ...
            //Act
            await _goalTrigger.Run(documentsList.AsReadOnly());

            //Assert
            _serviceBusClient.Verify(s => s.SendChangeFeedMessageAsync(It.IsAny<string>(), It.Is<ChangeFeedMessageModel>(m => m.IsGoal == true)));
        }
