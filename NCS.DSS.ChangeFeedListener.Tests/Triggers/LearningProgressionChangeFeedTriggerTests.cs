...
using Moq;using System.Text.Json;
...
{
    public class LearningProgressionChangeFeedTriggerTests
    {
        private Mock<ILogger<LearningProgressionChangeFeedTrigger.LearningProgressionChangeFeedTrigger>> _logger;
        
        private Mock<IChangeFeedListenerServiceBusClient> _serviceBusClient;
        private LearningProgressionChangeFeedTrigger.LearningProgressionChangeFeedTrigger _learningProgressionTrigger;
        private string _jsonDocument = string.Empty;

        [SetUp]
        public void Setup()
        {
            _logger = new Mock<ILogger<LearningProgressionChangeFeedTrigger.LearningProgressionChangeFeedTrigger>>();
            
            _serviceBusClient = new Mock<IChangeFeedListenerServiceBusClient>();
            _learningProgressionTrigger = new LearningProgressionChangeFeedTrigger.LearningProgressionChangeFeedTrigger(_serviceBusClient.Object,  _logger.Object);
        ...
        [Test]
        public async Task Run_CallsServiceBusClientMethodWithModelPropertyIsLearningProgressionAsTrue_WhenDocumentIsValid()
        {
            //Arrange
            var document = JsonConvert.DeserializeObject<JsonDocument>(_jsonDocument);

            var documentsList = new List<JsonDocument>
            {
                document
            };
            ...
            //Act
            await _learningProgressionTrigger.Run(documentsList.AsReadOnly());

            //Assert
            _serviceBusClient.Verify(s => s.SendChangeFeedMessageAsync(It.IsAny<string>(), It.Is<ChangeFeedMessageModel>(m => m.IsLearningProgression == true)));
        }
        ...
        public async Task Run_LogsInformation_WhenDocumentIsValid()
        {
            //Arrange
            var document = JsonConvert.DeserializeObject<JsonDocument>(_jsonDocument);
            var logMessage = string.Format("Attempting to send document id: {0} to service bus queue", document.RootElement.GetProperty("id"));
            ...
            _logger.Setup(l => l.LogInformation( It.IsAny<string>())).Verifiable();

            //Act
            await _learningProgressionTrigger.Run(documentsList.AsReadOnly());

            //Assert
            _logger.Verify(l => l.LogInformation( logMessage), Times.Once);
        }
        ...
        public async Task Run_LogsException_WhenServiceBusClientThrowsException()
        {
            ...
            //Assert
            _logger.Verify(l => l.LogError( logMessage, exception), Times.Once);
        }
    }
}
